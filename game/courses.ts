...
function hole(par: Hole["par"], distance: number, difficulty: number): Hole {
  return {
    ...
    distance,
    difficulty,
    wooded: Math.min(100, difficulty * 0.6),
    elevation: Math.min(100, difficulty * 0.4),
    obRisk: Math.min(100, difficulty * 0.5),
  };
}
